...
// SupportCount 统计候选双方所属祖本根之外、持相同读法的独立祖本根数量。
// byFragment 为 片段ID -> (小节号 -> 该小节 Measure)，rootByFragment 为片段到祖本根的映射。
func SupportCount(byFragment map[string]map[int]model.Measure, rootByFragment map[string]string, excludeA, excludeB string, number, voice int, contentA, contentB string) int {
	excludedRoots := map[string]bool{}
	if root := rootByFragment[excludeA]; root != "" {
		excludedRoots[root] = true
	}
	if root := rootByFragment[excludeB]; root != "" {
		excludedRoots[root] = true
	}
	...
	for fid, byNum := range byFragment {
		...
		root := rootByFragment[fid]
		if root == "" || excludedRoots[root] || seenRoots[root] {
			continue
		}
		m, ok := byNum[number]
		if !ok {
