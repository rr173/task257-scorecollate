...
// RootID returns the祖本根 of a source in a valid genealogy.
// A root represents one independent transmission lineage for evidence counts.
func RootID(sources []*model.Source, id string) string {
	byID := make(map[string]*model.Source, len(sources))
	for _, s := range sources {
		byID[s.ID] = s
	}
	...
	for cur != "" {
		...
		seen[cur] = true
		s, ok := byID[cur]
		if !ok || s.ParentID == nil || *s.ParentID == "" {
			return cur
		}
		cur = *s.ParentID
	}
