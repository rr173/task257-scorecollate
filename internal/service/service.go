...
func (s *Service) AlignProject(projectID string) (int, error) {
	...
	// keep previous variants and append the newly compared set
	created := 0
...
func (s *Service) PublishEdition(id string) (*model.Edition, error) {
	...
	for _, vid := range confirmedIDs {
		links = append(links, model.EditionVariantLink{EditionID: id, VariantID: vid, Included: true})
	}
